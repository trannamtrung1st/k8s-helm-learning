...
- **Helm (preferred):** **`./scripts/helm-apply.sh`** installs **`devops/workbench-crds-umbrella`** (RabbitMQ Cluster Operator), waits for the **`RabbitmqCluster`** CRD to be **Established**, then installs **`devops/workbench-umbrella`** (see **`devops/README.md`**). RabbitMQ and Redis config live under each chart’s **`files/`** directory (same paths → ConfigMap/Secret → volume mounts in cluster). **`workbench-rabbitmq`** on the main umbrella renders the **`RabbitmqCluster`** CR.
...
- **Layout:** Helm charts under **`devops/`** (**`crds/`**, **`platform/`**, **`infra/`**, **`apps/`**, **`workbench-crds-umbrella/`**, **`workbench-umbrella/`**). Legacy manifests live under **`devops/k8s/`** — see **[`devops/k8s/README.md`](../devops/k8s/README.md)**.
...
- **Postgres, RabbitMQ, Redis:** install with **Helm**. **RabbitMQ** uses **`workbench-crds-umbrella`** (Cluster Operator) plus **`workbench-rabbitmq`** on the main umbrella (`RabbitmqCluster` CR); **Postgres** and **Redis** use StatefulSet-based charts. Underlying controllers are often **StatefulSet** (operator-managed for RabbitMQ). You practice **StatefulSet** explicitly when you follow roadmap item **17** (inspect or hand-write manifests), not by reimplementing the database in app code.
