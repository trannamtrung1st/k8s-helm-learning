...
- **Helm (preferred):** **`./scripts/kind-e2e-first-run.sh`** (or manual cluster platform steps) installs **`./scripts/rabbitmq-install.sh`** (RabbitMQ Cluster Operator) before **`./scripts/helm-apply.sh`**, which installs **`devops/workbench-umbrella`** (see **`devops/README.md`**). RabbitMQ and Redis config live under each chart’s **`files/`** directory (same paths → ConfigMap/Secret → volume mounts in cluster). **`workbench-rabbitmq`** on the main umbrella renders the **`RabbitmqCluster`** CR.
...
- **Layout:** Helm charts under **`devops/`** (**`platform/`**, **`infra/`**, **`apps/`**, **`workbench-umbrella/`**). Legacy manifests live under **`devops/k8s/`** — see **[`devops/k8s/README.md`](../devops/k8s/README.md)**.
