...
| Job queue    | **RabbitMQ**                 | Job envelopes over **`RabbitMQ.Client`**. **Topology** is **not** created by the apps: **[`devops/infra/workbench-rabbitmq/files/definitions.json`](../devops/infra/workbench-rabbitmq/files/definitions.json)** plus boot import **[`devops/infra/workbench-rabbitmq/files/conf.d/10-workbench-definitions.conf`](../devops/infra/workbench-rabbitmq/files/conf.d/10-workbench-definitions.conf)**—**local Compose** bind-mounts both at broker boot; **Helm** and legacy **`devops/kustomization.yaml`** use the same files. **KEDA** can scale workers on queue depth or message rate in cluster deployments.                                                                                                                                                    |
...
- **Provisioning (v1):** create virtual host **`/`**, application user (**`workbench`** / **`workbench`** in this repo), and the job **exchange**, **queue**, and **binding** with a **definitions** JSON file imported at **node boot** (`definitions.import_backend = local_filesystem` and `definitions.local.path`, per [RabbitMQ](https://www.rabbitmq.com/docs/definitions)). The canonical file in this repo is **[`devops/infra/workbench-rabbitmq/files/definitions.json`](../devops/infra/workbench-rabbitmq/files/definitions.json)** with boot settings in **[`devops/infra/workbench-rabbitmq/files/conf.d/10-workbench-definitions.conf`](../devops/infra/workbench-rabbitmq/files/conf.d/10-workbench-definitions.conf)**—**local Compose** bind-mounts both (see **[`local/docker-compose.infra.yaml`](../local/docker-compose.infra.yaml)**); **Helm** embeds them via **`.Files.Get`**; legacy **Kustomize** uses **`devops/kustomization.yaml`** (**`configMapGenerator`**). Boot import **does not** seed Docker’s default user: credentials and topology live in that JSON. **Do not rely on apps declaring topology.** The **API** and **worker** only connect, publish, and consume.
...
- **Helm (preferred):** **`./scripts/kind-e2e-first-run.sh`** (or manual cluster platform steps) installs **`./scripts/rabbitmq-install.sh`** (RabbitMQ Cluster Operator) before **`./scripts/helm-apply.sh`**, which installs **`devops/umbrellas/workbench-umbrella`** (see **`devops/README.md`**). RabbitMQ and Redis config live under each chart’s **`files/`** directory (same paths → ConfigMap/Secret → volume mounts in cluster). **`workbench-rabbitmq`** on the main umbrella renders the **`RabbitmqCluster`** CR.
- **Legacy Kustomize:** **`kubectl apply -k devops`** (or **`kustomize build devops | kubectl apply -f -`**) still works for reference; **`devops/kustomization.yaml`** generators read **`infra/workbench-rabbitmq/files/`** and **`infra/workbench-redis/files/`**. **GitOps** tools (**Argo CD**, **Flux**) can point at Helm or the Kustomize root later (roadmap item **48**).
- **Layout:** Helm charts under **`devops/`** (**`platform/`**, **`infra/`**, **`apps/`**, **`umbrellas/workbench-umbrella/`**). Legacy manifests live under **`devops/k8s/`** — see **[`devops/k8s/README.md`](../devops/k8s/README.md)**.
...
- **Postgres, RabbitMQ, Redis:** install with **Helm**. **RabbitMQ** uses **`./scripts/rabbitmq-install.sh`** (Cluster Operator) plus **`workbench-rabbitmq`** on the main umbrella (`RabbitmqCluster` CR); **Postgres** and **Redis** use StatefulSet-based charts. Underlying controllers are often **StatefulSet** (operator-managed for RabbitMQ). You practice **StatefulSet** explicitly when you follow roadmap item **17** (inspect or hand-write manifests), not by reimplementing the database in app code.
