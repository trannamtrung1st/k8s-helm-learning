...
| Job queue    | **RabbitMQ**                 | Job envelopes over **`RabbitMQ.Client`**. **Topology** is **not** created by the apps: **[`devops/rabbitmq/definitions.json`](../devops/rabbitmq/definitions.json)** plus boot import **[`devops/rabbitmq/conf.d/10-workbench-definitions.conf`](../devops/rabbitmq/conf.d/10-workbench-definitions.conf)**—**local Compose** bind-mounts both at broker boot; the same paths are wired into the cluster via **`devops/kustomization.yaml`** (**`configMapGenerator`**). **KEDA** can scale workers on queue depth or message rate in cluster deployments.                                                                                                                                                    |
...
- **`kubectl apply -k devops`** (or **`kustomize build devops | kubectl apply -f -`**) is the day-to-day render path for the **full** stack in this repo (includes **`configMapGenerator`** / **`secretGenerator`** for **`devops/rabbitmq/`** and **`devops/redis/`**); **GitOps** tools (**Argo CD**, **Flux**) can point at the same **`kustomization.yaml`** root later (roadmap item **48**).
...
- **Layout:** a reference tree for **`apps/`** (per-service `base/` + `overlays/`), **`infrastructure/`**, **`clusters/`**, **`platform/`**, and **`scripts/`** is in **[`devops/k8s/README.md`](../devops/k8s/README.md)**. Manifests live under **`devops/k8s/`**; the **Kustomize apply root** is **`devops/`** (**`devops/kustomization.yaml`**, including generators that read **`devops/rabbitmq/`** and **`devops/redis/`**).
