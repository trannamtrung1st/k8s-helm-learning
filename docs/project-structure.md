...
- **Kubernetes / Helm:** charts live under **`devops/`** (**`platform/`**, **`infra/`**, **`apps/`**, **`umbrellas/workbench-umbrella/`**; chart dirs use the **`workbench-`** prefix where applicable, e.g. **`platform/workbench-namespaces/`**). **Apply** cluster platform prerequisites (Istio, RabbitMQ operator), then the stack with **`./scripts/helm-apply.sh`** (see **[`devops/README.md`](../devops/README.md)**). Legacy Kustomize manifests under **`devops/k8s/`** are documented in **[`devops/k8s/README.md`](../devops/k8s/README.md)** (**`devops/kustomization.yaml`** is deprecated).
