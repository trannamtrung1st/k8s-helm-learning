...
- **Kubernetes / Helm:** charts live under **[`devops/helm/`](../devops/helm/)** (**`platform/`**, **`infra/`**, **`apps/`**, **`workbench-umbrella/`**; chart dirs use the **`workbench-`** prefix, e.g. **`platform/workbench-namespaces/`**). **Apply** the full stack with **`./scripts/helm-apply.sh`** (see **[`devops/helm/README.md`](../devops/helm/README.md)**). **`devops/rabbitmq/`** and **`devops/redis/`** remain canonical; RabbitMQ/Redis charts embed them via **`files/`** symlinks. Legacy Kustomize manifests under **`devops/k8s/`** are documented in **[`devops/k8s/README.md`](../devops/k8s/README.md)** (**`devops/kustomization.yaml`** is deprecated).
